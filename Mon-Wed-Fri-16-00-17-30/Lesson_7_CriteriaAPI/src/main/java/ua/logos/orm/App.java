...
// Teacher teacher = em.createQuery("SELECT t FROM Teacher t WHERE t.id = :id",Teacher.class)
		...
		Root<Course> root = query.from(Course.class); // FROM Course c
		query.select(root); // SELECT c FROM Course c
		
		// SELECT c FROM Course c WHERE c.price
//		Expression<BigDecimal> priceExpresion = root.get("price");
		
		// SELECT c FROM Course c WHERE c.price >= 50.00
//		Predicate pricePredicate = cb.ge(priceExpresion, new BigDecimal("50.00"));
//		Predicate pricePredicat2 = cb.lt(priceExpresion, new BigDecimal("70.00"));
//		
//		Predicate pricePredicat3 = cb.between(priceExpresion, new BigDecimal("50.99"), new BigDecimal("90.99"));
//		
		// SELECT c FROM Course c WHERE c.price > 50.00 AND c.price < 70.00
//		Predicate allPredicates = cb.and(pricePredicate, pricePredicat2);
		
		// query.where(allPredicates);
	
//		Join<Course, Teacher> joinTeacher = root.join("teacher");
//		Expression<Long> teacherExpresion = joinTeacher.get("id");
//		Predicate teacherPredicat = cb.equal(teacherExpresion, new Long(5));
				
//		query.where(pricePredicat3);
		
		Expression<String> titleExpression = root.get("title");
		Predicate titlePredicate = cb.like(titleExpression, "%6");
		
		Expression<Long> idExpresion = root.get("id");
		Predicate idPredicate = cb.ge(idExpresion, new Long(10)); 
		Predicate allPredicates = cb.and(titlePredicate, idPredicate);
